...
// Attention : ici yVertical est l'axe vertical et zLateral est l'axe latéral.
struct Position {
  double x;         // X plan horizontal
  double yVertical; // Y vertical
  double zLateral;  // Z latéral
};

// Définir les positions dans votre repère : {x, yVertical, zLateral}
Position pos1 = {-75, -120, -30};
Position pos2 = {-75, -120, -65};  // mouvement latéral seul
Position pos3 = {50, -140, -30};  // mouvement en X + vertical
...
void setup() 
{
  ...
  dxl.writeControlTableItem(MOVING_SPEED, 1, 100);  // Vitesse réduite pour le moteur 1 0 vite -> 1023 lent
  dxl.writeControlTableItem(MOVING_SPEED, 2, 100);
  dxl.writeControlTableItem(MOVING_SPEED, 3, 100);
}
...
// Convertit les coordonnées de votre repère (Y vertical, Z latéral)
// vers la cinématique du delta qui attend Z vertical.
void moveTo(double x, double yVertical, double zLateral)
{
  soft_serial.print("EXT x="); soft_serial.print(x);
  soft_serial.print(" yVertical="); soft_serial.print(yVertical);
  soft_serial.print(" zLateral="); soft_serial.print(zLateral);
  soft_serial.print(" -> ");
...
void loop() 
{ 
  moveTo(pos1.x, pos1.yVertical, pos1.zLateral);
  delay(1000);
  moveTo(pos2.x, pos2.yVertical, pos2.zLateral);
  delay(1000);
  digitalWrite(4, HIGH);
  delay(200);
  moveTo(pos1.x, pos1.yVertical, pos1.zLateral);
  delay(1400);
  moveLinear(pos1.x, pos1.yVertical, pos1.zLateral,
             pos3.x, pos3.yVertical, pos3.zLateral,
             20, 50);
  digitalWrite(4, LOW);
  delay(5000);
}
...
void calculeEtAffiche(double x, double y, double z) 
{
  ...
  // Affiche pour débogage interne du delta (x, y, z vertical)
  soft_serial.print("DELTA x="); soft_serial.print(x);
  soft_serial.print(" y="); soft_serial.print(y);
  soft_serial.print(" z="); soft_serial.print(z);
