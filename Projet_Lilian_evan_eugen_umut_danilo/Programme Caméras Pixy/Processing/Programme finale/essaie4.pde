...
// DESSIN DES SLIDERS
// ============================================
void dessinerSliders() {
  // Fond panneau
  fill(15, 15, 28, 220);
  noStroke();
  rect(sliderX + 340, 45, sliderW + 20, 300, 10);

  // Titre panneau
  fill(200, 200, 255);
  textSize(12);
  textAlign(LEFT);
  text("REGLAGES", sliderX + 350, 65);

  // On dessine chaque slider manuellement
  dessinerUnSlider(0, sSEUIL,   minSEUIL, maxSEUIL, "SEUIL",   sliderX, sliderY + 20,  " px",   true);
  dessinerUnSlider(1, sTIMEOUT, minTIME,  maxTIME,  "TIMEOUT", sliderX, sliderY + 70,  " ms",   true);
  dessinerUnSlider(2, sFANTOME, minFAN,   maxFAN,   "FANTOME", sliderX, sliderY + 120, " ms",   true);
  dessinerUnSlider(3, sTAILLE,  minTAI,   maxTAI,   "TAILLE",  sliderX, sliderY + 170, " px",   true);
  dessinerUnSlider(4, sPX2M,    minPX,    maxPX,    "Pixel en m/s",    sliderX, sliderY + 220, " m/px", false);
}

void dessinerUnSlider(int idx, float val, float min, float max, String label, int x, int y, String unite, boolean entier) {
  // Label + valeur
  fill(180, 200, 255);
  textSize(11);
  textAlign(LEFT);
  String valStr = entier ? str(round(val)) : nf(val, 1, 4);
  text(label + " : " + valStr + unite, x+350, y+10);

  // Barre de fond
  fill(40, 40, 60);
  noStroke();
  rect(x+350, y + 20, sliderW, sliderH, 4);

  // Barre remplie (ratio entre min et max)
  float ratio = (val - min) / (max - min);
  fill(100, 140, 255);
  rect(x+350, y + 20, sliderW * ratio, sliderH, 4);

  // Poignée
  float px = x + sliderW * ratio;
  fill(220, 230, 255);
  stroke(100, 140, 255);
  strokeWeight(2);
  ellipse(px+350, y + 20 + sliderH/2, 16, 16);
  noStroke();
}
...
// ============================================
void mousePressed() {
  int offsetX = sliderX + 350; // On applique le même décalage que le dessin
  
  for (int i = 0; i < 5; i++) {
    // Calcul de la position Y du rectangle (y + 20 dans ton code de dessin)
    int yBarre = (sliderY + 20 + i * 50) + 20; 
    
    // On vérifie si la souris est dans le rectangle du slider
    if (mouseX >= offsetX && mouseX <= offsetX + sliderW &&
        mouseY >= yBarre && mouseY <= yBarre + sliderH) {
      sliderActif = i;
    }
  }
}
...
void mouseDragged() {
  if (sliderActif < 0) return;

  int offsetX = sliderX + 350;
  // Calcul du ratio (0.0 à 1.0) basé sur la nouvelle position X
  float ratio = constrain((float)(mouseX - offsetX) / sliderW, 0, 1);

  // Mise à jour des variables
  if (sliderActif == 0) sSEUIL   = minSEUIL + ratio * (maxSEUIL - minSEUIL);
  if (sliderActif == 1) sTIMEOUT = minTIME  + ratio * (maxTIME  - minTIME);
  if (sliderActif == 2) sFANTOME = minFAN   + ratio * (maxFAN   - minFAN);
  if (sliderActif == 3) sTAILLE  = minTAI   + ratio * (maxTAI   - minTAI);
  if (sliderActif == 4) sPX2M    = minPX    + ratio * (maxPX    - minPX);
}
